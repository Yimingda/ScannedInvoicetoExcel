...
- **带税总额判定**：
  ...
  - **手写含小费终额**：识别「印刷 Total → Gratuity:(留空) → TOTAL:(手写)」的餐厅版式，
    优先取小费行之后的终额（限 0.6~1.6× 印刷值内，置信度压为 medium 强制人工过目）。
  - 刷卡实付（Tendered/C_CARD）> 票面总额且在小费比例内 → 以实付为准，优先级最高。
- **刷卡回执 = 金额验证器**：回执与发票金额精确一致 → 记录升为高置信并标「已互验」；
  小费付现金的版式（回执 ≈ 发票的印刷总额、手写终额=印刷+小费）也能识别并合并，
  金额取含小费终额。
- **人工纠错闭环**（确认向导内）：
  - 「删除本条」剔除误识别记录（可恢复，不导出不计数）；
  - 「拆成 2/3 张重识别」——自动分割把多张票并成一张时，对该区域强制均分重跑 OCR，
    替换为多条独立记录。
