...
浏览器打开 `http://localhost:8501`：拖入发票 → 点「开始识别」→ 表格里每张小票**带原图缩略图**，
`⚠️` 行是程序没把握的，对着图直接改 **日期/金额** → 点「下载报销导入表」。左侧栏可调 DPI、
日期读法、去重开关，也可上传你自己的 Excel 模板（需与默认模板同结构）。
...
## 实测准确率（2026-03 南非餐饮/高尔夫票据，5 页 16 张）

| 指标 | 结果 | 说明 |
|---|---|---|
| 带税金额命中 | **13/16** | 3 个未中全部是**物理叠贴+倾斜+手写批注**的小票（第 4、5 页），OCR 串行成乱码 |
| 日期（金额对的里） | 7/13 | 其中 1 条实为你表里的笔误（票面 30 号、你填了 20 号），1 条 OCR 年份误读已自动标黄 |
| 发票+刷卡回执去重 | 正确 | 480/660/5500 等「票面+小费=刷卡实付」均正确合并取实付 |

**结论**：干净、单张成页的票据日期金额都稳；难点是一页里多张小票**互相叠压/歪斜/手写划改**的情形——
这是本地 OCR 的物理极限。这类页面建议：①扫描时把小票摊平不重叠；②或对该页单独提高 DPI；
③复核表红色行人工补录。
