...
_TXT_FIXES = [
    ...
    # 排除后接字母词（"1. 25 Main Road" 是地址）与后接数字（"Jul 5. 2026"
    # 是日期，吞并会毁掉年份）的场景
    (re.compile(r"(\d)\.\s+(\d{2})(?!\d)(?!\s*[A-Za-z])"), r"\1.\2"),
    ...
    # 币种符号后的 I/l/! 是 1：RI,164.00 -> R1,164.00（笔划污损常见）
    (re.compile(r"(?<=[R$€£¥])[Il!](?=[\d,.])"), "1"),
    # 千分位逗号后的 I/l 是 1：1,I64.00 -> 1,164.00
    (re.compile(r"(?<=,)[Il](?=\d{2})"), "1"),
    # 千分位分隔被读成点/双逗号：1.164.00 -> 1,164.00；1,164,00 -> 1,164.00
    # （x.yyy.zz / x,yyy,zz 不是任何合法单一金额写法，改写安全）
    (re.compile(r"(\d)\.(\d{3})\.(\d{2})\b"), r"\1,\2.\3"),
    (re.compile(r"(\d),(\d{3}),(\d{2})\b"), r"\1,\2.\3"),
    # 千分位逗号后被 OCR 挤进空格：R4, 322.00 -> R4,322.00（后接小数/边界才改，
    # 「Qty 2, 300ml」这类后接字母的不动）
    (re.compile(r"(\d),\s+(\d{3})(?=\.\d{2}\b|\s|$)"), r"\1,\2"),
]
